...
#         super(TDSGRUEncoder, self).__init__()
...
class RNNLayer(nn.Module):
    """
    Small helper module that takes (T, N, in_size) as input,
    runs it through an LSTM, and outputs (T, N, num_classes)
    ...
    def __init__(self, in_size: int, hidden_size: int, num_layers: int,out_ll_size: int = 384 ):
        ...
        # #self.fc = nn.Linear(hidden_size * 2, charset().num_classes)
        # self.log_softmax = nn.LogSoftmax(dim=-1)
        self.norm = nn.LayerNorm(hidden_size * 2)  # Add LayerNorm
